...
import deleteCard from './components/Card/Card';



function App() {
    const [lists, setLists] = useState(DB.lists.map(item => {item.color = DB.colors.filter(color => color.id === item.colorId)[0].name;
        return item;
    }))
    const [tasks, setTasks] = useState(DB.tasks.map(item => {item.task = DB.lists.filter(task => task.listId === lists.id)[0].id;
  return item;}))
    ...
    <div className="todo">
        <div className="todo_sidebar">
            ...
            <List
                ...
                deleteCard={list => {console.log(list)}}
            />
        ...
        <div className="todo_tasks">
            {Number(activeId) ? <Card  tasks={tasks} activeItem={activeId} lists={lists} setLists={setLists} setActiveId={setActiveId}/> : <div className="backGround"> Zametki4000</div>}
        </div>
    </div>
  );
}
